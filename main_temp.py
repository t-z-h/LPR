...
def find_position(car_pic):
    ...
    for card_index, card_img in enumerate(card_imgs):
        ...
        if xl >= xr:
            ...
            need_accurate = True
        # 截取车牌精确位置
        print(yh, yl, (yh - yl) // 4, xl, xr)
        show_img(card_img)
        # 因为在上面进行了以颜色对车牌的精确定位，会导致绿色车牌上半部分的颜色渐变区域没有取到，所以在此要做一个判断
    ...
    for i, color in enumerate(colors):
        if color in ("blue", "yello", "green"):
            ...
            if len(wave_peaks) <= 6:
                ...
                continue
            # print("before wave_peaks", wave_peaks)
            # 认为垂直方向，最大的波峰为车牌区域
            wave = max(wave_peaks, key=lambda x: x[1] - x[0])
            # print("wave", wave)
            # 渠道长度最大的波峰
            ...
            if len(wave_peaks) <= 6:  # 如果所有字符长度小于6，说明他不是车牌(车牌的长度应该大于6)
                print("字符数量不够:", len(wave_peaks))
                continue
            ...
            for i, part_card in enumerate(part_cards):
                ...
                # if i == 0 and np.sum(part_card[:44, :2]) / 255 > 10:
                    # print("k", np.sum(part_card[:44, :2]) / 255)
                    # part_card[:44, :2] = 0

                # print(part_card[:44, :3])
                ...
                # img = cv2.resize(part_card, (SZ, SZ), interpolation=cv2.INTER_AREA)
        ...
        try:
            ...
            show_img(card_img)
            return res, card_color, card_img
        except:
            print("no")
...
colors, card_imgs = find_position("./img/皖AUB816.jpg")
